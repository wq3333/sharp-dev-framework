...
public class DataCleanupTask(IServiceProvider serviceProvider) : BaseTask(serviceProvider)
{
    ...
    async Task CleanupTableAsync(DataCleanupTableConfig config, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            deletedCount = tableName switch
            {
                "UserOperationLogs" => await _dbContext.UserOperationLogs
                                        .Where(x => x.CreatedAt < cutoffTimestamp)
                                        .ExecuteDeleteAsync(cancellationToken),
                "Tasks" => await _dbContext.Tasks
                                        .Where(x => x.CreatedAt < cutoffTimestamp && (x.Status == TaskStates.Completed || x.Status == TaskStates.Cancled))
                                        .ExecuteDeleteAsync(cancellationToken),
                _ => await _dbContext.Database.ExecuteSqlInterpolatedAsync(
                                        $"DELETE FROM {tableName} WHERE CreatedAt < {cutoffTimestamp}",
                                        cancellationToken),
            };
            if (_logger.IsEnabled(Microsoft.Extensions.Logging.LogLevel.Information))
            {
                _logger.LogInformation("数据清理完成: 表={TableName}, 删除记录数={DeletedCount}", tableName, deletedCount);
            }
        }
        catch (Exception ex)
        {
            ...
            {
                _logger.LogError(ex, "数据清理失败: 表={TableName}", tableName);
            }
        }
    }
}
