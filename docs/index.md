...
| 项目 | 类型 | 说明 |
|------|------|------|
| `SharpDevFramework` | 类库 (NuGet) | 框架核心，版本 1.0.12 |
...
public class DemosController(AppDbContext context, TaskCenter taskCenter) : ControllerBase
{
    ...
    {
        var demo = context.Demos.Find(id) ?? throw new KnownException("data not found");
...
- 数据不存在时 `throw new KnownException("data not found")`（被 ExceptionFilter 捕获返回 500）
...
public class XxxsController(AppDbContext context) : ControllerBase
{
    ...
    {
        var entity = context.Xxxs.Find(id) ?? throw new KnownException("data not found");
...
public class MyNewTask(IServiceProvider serviceProvider) : BaseTask(serviceProvider)
{
    ...
    {
        var task = _dbContext.Tasks.Find(id) ?? throw new KnownException($"task not found with id:{id}");
...
| 使用 `throw new KnownException()` 但期望返回特定 HTTP 状态码 | 框架 ExceptionFilter 将所有非 `UnauthorizedAccessException` 返回 500；`UnauthorizedAccessException` 返回 403 |
